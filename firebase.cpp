    ...
    if (fbdo.dataType() == "json")
    {
      ...
      case 'i': // estado "iniciando"
        ...
        if (digitalRead(PINO_D18) == HIGH)
        {
          ...
          // Calcula a distância com base no avanço da rosca (igual código offline)
          float distanciaBruta = abs(pulsos) * MM_POR_PULSO;
          distanciaFinal = 124.5 - distanciaBruta; // 125 é o valor fixo do avanço total (como no offline)
          encoderPulsos = 0;
          // Imprime os dados no monitor serial
          Serial.print("Pulsos contados: ");
          Serial.println(pulsos);
          Serial.print("Distância final calculada: ");
      ...
      case 'p': // estado "parado"
      {
        ...
        if (Firebase.RTDB.getString(&fbdo, "/BrocaSelecionadaID/id"))
        {
          ...
          {
            adicionarNovoCompParaBroca(selectedBrocaID, distanciaFinal);
            delay(1000);
          }
          alterarEstado("c");
        }
